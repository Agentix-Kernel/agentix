...
| `driver` | Builtin factory key (`anthropic`, `huble`, `melious`, `openai-embedding`, `huble-embedding`, `hf-stt`, `minio-object-store`, `sqlite-relational`) or a dotted path `pkg.mod:Class` (seam #13, [`drivers.md`](drivers.md) §6). |
