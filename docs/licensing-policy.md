...
Two tiers. **Closed/proprietary** for the engine + product control-plane; **source-available**
(BSL 1.1 → Apache-2.0) for the clients + the gateway edge. BSL ≠ OSI open-source: source is visible +
modifiable for **non-production** use; production needs a commercial license until the Change Date,
when it converts to Apache-2.0.

| Repo (dir) | GitHub | License |
|---|---|---|
| ludo-agent | euroblaze/ludo | **Proprietary** — all rights reserved |
...
## Known conflict to fix (CRIE)
`ludo-agent/pyproject.toml` declares `license = { text = "MIT" }` — **wrong**; ludo-agent is
proprietary. Correct it to proprietary metadata (flagged follow-up; not auto-changed here).
